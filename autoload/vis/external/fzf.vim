...
func vis#external#fzf#bmk()
  ...
  let options += ['--header', '[A-o] Open, [A-t] Preview']
  let options += ['--bind', 'alt-o:execute(open_bmk.sh {})']
  let options += ['--bind', 'alt-t:change-preview-window(down,75%|hidden|)']
...
func vis#external#fzf#fd(type, sink)
  ...
  let options += ['--ansi']
  let options += ['--info', 'inline-right']
  let options += ['--header', '[A-adf] All|Dir|File, [A-xcgv] Explorer|Chrome|Gvim|Vscode, [A-t] Preview']
